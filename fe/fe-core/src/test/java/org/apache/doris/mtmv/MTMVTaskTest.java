...
public class MTMVTaskTest {
    ...
    public void testTaskInfoContainsIvmFallbackReasonColumn() {
        ...
        Deencapsulation.setField(task, "ivmFallbackReason", IvmFallbackReason.BINLOG_NOT_ENABLED.name());
        ...
        Assert.assertEquals(IvmFallbackReason.BINLOG_NOT_ENABLED.name(), cells.get(columnIndex).getStringVal());
    }
    ...
    public void testTryIvmFastPathRecordsFallbackReason() throws Exception {
        ...
        try (MockedConstruction<IvmRefreshManager> ignored = Mockito.mockConstruction(IvmRefreshManager.class,
                (mock, context) -> Mockito.when(mock.doRefresh(mtmv)).thenReturn(
                        IvmRefreshResult.fallback(IvmFallbackReason.BINLOG_NOT_ENABLED, "no_binlog")))) {
            Assert.assertFalse(Deencapsulation.invoke(task, "tryIvmFastPath"));
        }

        Assert.assertEquals(IvmFallbackReason.BINLOG_NOT_ENABLED.name(),
