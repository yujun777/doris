...
public class ReportHandler extends Daemon {
    ...
    private static void sync(Map<Long, TTablet> backendTablets, Map<Long, ListMultimap<Long, Long>> tabletSyncMap,
        ...
        for (Long dbId : tabletSyncMap.keySet()) {
            ...
            for (int i = 0; i < tables.size(); i += batchTableSize) {
                BatchModifyReplicasInfo batchInfo = new BatchModifyReplicasInfo();
                int toIdx = Math.min(i + batchTableSize, tables.size());
                List<Table> existTables = MetaLockUtils.writeLockTablesIfExist(tables.subList(i, toIdx));
                try {
                    for (Table table : existTables) {
                        ...
                        for (int j = 0; j < tabletMetaList.size(); j++) {
                            ...
                            if (syncTablet(tabletId, tabletMeta, olapTable, backendId, backendTablets, batchInfo)) {
                                syncCounter++;
                            }
                            tryWriteBatchModifyReplica(batchInfo, false);
                        }
                    }
                    tryWriteBatchModifyReplica(batchInfo, true);
                } finally {
    ...
    private static void tryWriteBatchModifyReplica(BatchModifyReplicasInfo batchInfo, boolean force) {
        int size = batchInfo.getReplicas().size();
        if (size == 0) {
            return;
        }

        if (force || size >= Config.report_tablet_editlog_batch_size) {
            Env.getCurrentEnv().getEditLog().logBatchModifyReplica(batchInfo);
            batchInfo.clear();
        }
    }

    private static boolean syncTablet(long tabletId, TabletMeta tabletMeta, OlapTable olapTable, long backendId,
        Map<Long, TTablet> backendTablets, BatchModifyReplicasInfo batchInfo) {
                ...
                try {
                    for (Table table : existTables) {
                        ...
                        for (int j = 0; j < tabletMetaList.size(); j++) {
                            TabletMeta tabletMeta = tabletMetaList.get(j);
                            long tabletId = tabletIds.get(j);
                            if (deleteTabletFromMeta(tabletId, tabletMeta, olapTable, backendId, backendReportVersion,
                                        backendHealthPathHashs, createReplicaBatchTask, batchInfo)) {
                                deleteCounter++;
                            }
                            tryWriteBatchModifyReplica(batchInfo, false);
                        }
    ...
    private static boolean deleteTabletFromMeta(long tabletId, TabletMeta tabletMeta, OlapTable olapTable,
            long backendId, long backendReportVersion, Set<Long> backendHealthPathHashs,
            AgentBatchTask createReplicaBatchTask, BatchModifyReplicasInfo batchInfo) {
        ...
        if (backendReportVersion < currentBackendReportVersion) {
            ...
            boolean existsOtherHealthReplica = tablet.getReplicas().stream()
                .anyMatch(r -> r.getBackendIdWithoutException() != replica.getBackendIdWithoutException()
    ...
    private static boolean addReplica(long tabletId, TabletMeta tabletMeta, TTabletInfo backendTabletInfo,
            long backendId, OlapTable olapTable, BatchModifyReplicasInfo batchInfo) {
        if (tabletMeta == null) {
            return false;
        }
