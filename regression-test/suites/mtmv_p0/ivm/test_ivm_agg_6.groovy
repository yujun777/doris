...
suite("test_ivm_agg_6") {
    ...
    sql """drop materialized view if exists ivm_agg_mm_zero_mv;"""
    sql """drop table if exists ivm_agg_mm_zero_base;"""

    sql """
        CREATE TABLE ivm_agg_mm_zero_base (
            k1 INT,
            v1 INT
        )
        UNIQUE KEY(k1)
    ...
    sql """
        INSERT INTO ivm_agg_mm_zero_base VALUES
            (1, 10),
            (2, 20);
    """
    ...
    order_qt_minmax_zero_after_complete """
        SELECT min_v1, max_v1, cnt, sum_v1 FROM ivm_agg_mm_zero_mv
    """

    // Step 2: Delete ALL rows via binlog_op=1
    // Non-null count for MIN/MAX drops to 0 → guard bypassed → MIN/MAX = NULL
    sql """DELETE FROM ivm_agg_mm_zero_base WHERE k1 = 1;"""
    sql """DELETE FROM ivm_agg_mm_zero_base WHERE k1 = 2;"""
    ...
    sql """drop materialized view if exists ivm_agg_mm_nullkeep_mv;"""
    sql """drop table if exists ivm_agg_mm_nullkeep_base;"""

    sql """
        CREATE TABLE ivm_agg_mm_nullkeep_base (
            k1 INT,
            v1 INT
        )
        UNIQUE KEY(k1)
    ...
    sql """INSERT INTO ivm_agg_mm_nullkeep_base VALUES (1, 10);"""
    sql """INSERT INTO ivm_agg_mm_nullkeep_base (k1, v1) VALUES (2, NULL);"""
    ...
    order_qt_nullkeep_after_complete """
        SELECT min_v1, max_v1, cnt, sum_v1 FROM ivm_agg_mm_nullkeep_mv
    """
    ...
    sql """DELETE FROM ivm_agg_mm_nullkeep_base WHERE k1 = 1;"""
