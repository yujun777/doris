...
suite("test_ivm_drop_base_table_stream_state") {
    sql "DROP MATERIALIZED VIEW IF EXISTS ivm_drop_base_stream_state_mv"
    ...
    sql """
        CREATE MATERIALIZED VIEW ivm_drop_base_stream_state_mv
        BUILD DEFERRED REFRESH INCREMENTAL ON MANUAL
        DISTRIBUTED BY RANDOM BUCKETS 1
        PROPERTIES ("replication_num" = "1")
        AS SELECT id, value FROM ivm_drop_base_stream_state_base
    """

    sql "REFRESH MATERIALIZED VIEW ivm_drop_base_stream_state_mv INCREMENTAL"
    waitingMTMVTaskFinishedByMvName("ivm_drop_base_stream_state_mv")

    order_qt_initial_mv_rows """
        SELECT id, value
        FROM ivm_drop_base_stream_state_mv
        ORDER BY id
    """
